...
- Providers with subprocess transports test `buildInvocation`/`parseOutput` as pure functions. The X companion source tests the whole silent-failure contract through `runSearch` with fake binaries.
