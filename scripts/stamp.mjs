...
// run.ps1, and references/runtime.md can never drift from package.json by hand.
// The drift test (scripts/stamp.test.mjs) reads the same three files back and
// asserts they match. Keeping the read and the write in one module is what
...
/**
 * The three files that carry the pinned version, each with a pattern that
 * matches the whole version line and captures just the value in group 1, plus
 * a formatter that rebuilds that line for a given version. One pattern serves
 * both stamping (replace the line) and reading (capture the value).
 */
