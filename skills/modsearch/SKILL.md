...
Some harnesses forbid running scripts. Reason through the same order by hand and run the first line that works (the pinned version is 5.2.0):

1. A `modsearch` on `PATH` whose major version is 5 and is at least 5.2.0: `modsearch <args>`.
2. Otherwise, if `npx` exists: `npx --yes --package @liustack/modsearch@5.2.0 modsearch <args>`.
3. Otherwise, if `bunx` exists: `bunx --bun @liustack/modsearch@5.2.0 <args>`.
