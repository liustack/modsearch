...
- Both READMEs' installation section now runs in execution order: step 1 is the only human part, preparing a search engine (install Antigravity and sign in, or register one free key), and step 2 hands one line to the user's AI, which follows `INSTALL.md` and reports back. The hero one-liner above the fold is removed with it. The CLI usage that lived in the README moves to `skills/modsearch/references/cli.md`, since the CLI is driven by the agent, and the README keeps a one-line pointer in the docs table.
